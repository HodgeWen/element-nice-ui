import { arrayFindIndex } from 'element-nice-ui/src/utils/util'
import { getCell, getColumnByCell, getRowIdentity } from './util'
import { getStyle, hasClass, removeClass, addClass } from 'element-nice-ui/src/utils/dom'
...
export default {
  ...
  render(h) {
    ...
    return (
      <table class='el-table__body' cellspacing='0' cellpadding='0' border='0'>
        ...
        <tbody>
          {data.map((row, index) => this.wrappedRowRender(row, index))}
          {/* {data.reduce((acc, row) => {
  ...
  methods: {
    ...
    rowRender(row, $index, treeRowData, indexPath) {
    ...
    wrappedRowRender(row, $index) {
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        const tr = this.rowRender(row, $index, null, [$index])
        if (!renderExpanded) {
        ...
        const tmp = [this.rowRender(row, $index, treeRowData, [$index])]
        // 渲染嵌套数据
        if (cur) {
          // currentRow 记录的是 index，所以还需主动增加 TreeTable 的 index
          // let i = 0
          const traverse = (children, parent, indexPath) => {
            ...
            children.forEach((node, childIndex) => {
              ...
              // i++
              let childIndexPath = indexPath.concat(childIndex)
              tmp.push(this.rowRender(node, $index, innerTreeRowData, childIndexPath))
              if (cur) {
                const nodes = lazyTreeNodeMap[childKey] || node[childrenColumnName]
                traverse(nodes, cur, childIndexPath)
              }
            })
          }
          ...
          traverse(nodes, cur, [$index])
        }
        return tmp
      } else {
        return this.rowRender(row, $index, null, [$index])
      }
    }
  }
}
