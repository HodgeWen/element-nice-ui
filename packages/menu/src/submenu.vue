...
export default {
    ...
    isFirstLevel() {
      ...
      while (parent && parent !== this.rootMenu) {
        ...
        if (
          ...
          (menuData && menuData.children && menuData.children.length)
        ) {
          isFirstLevel = false
          break
        } else {
  ...
  methods: {
    ...
    handleMouseenter(event, showTimeout = this.showTimeout) {
      ...
      // 此处修复栈溢出的bug
      if (this.appendToBody && event.target !== this.$parent.$el) {
  ...
  render(h) {
    ...
    const { size } = this.rootMenu
    let classes = ['el-menu el-menu--popup', `el-menu--popup-${currentPlacement}`]
    ;['small', 'large', 'medium'].includes(size) && classes.push(`el-menu--${size}`)
