...
import { modifierMethod, components, form } from './utils'
...
export default {
  ...
  methods: {
    ...
    initForm() {
      const { originModel, originRules } = this

      let model = this.originModel
      let rules = this.originRules
      if ((!originModel || !originRules) && this.form) {
        const formModel = form(this.form, this.trigger)
